...
class ALGame(gym.Env):
    ...
    def _cache_features(self):
        """Pre-extract backbone features once so training only touches the linear head."""
        from classifiers.classifier import DINOv3Classifier
        if not isinstance(self.classifier, DINOv3Classifier):
            return
        if not self.classifier.freeze_backbone:
            return

        print("Pre-extracting DINOv3 features (one-time cost)...")
        batch_size = self.dataset.classifier_batch_size

        self.dataset.x_train = self.classifier.extract_features(
            self.dataset.x_train, batch_size).to(self.device)
        self.dataset.x_test = self.classifier.extract_features(
            self.dataset.x_test, batch_size).to(self.device)
        self.dataset.x_val = self.classifier.extract_features(
            self.dataset.x_val, batch_size).to(self.device)
        self.dataset.x_labeled = self.classifier.extract_features(
            self.dataset.x_labeled, batch_size).to(self.device)
        self.dataset.x_unlabeled = self.classifier.extract_features(
            self.dataset.x_unlabeled, batch_size).to(self.device)

        self.dataset.x_shape = self.dataset.x_train.shape[1:]

        self.classifier.enable_feature_cache()
        ...
        self.reset_al_pool()
        print(f"Feature caching complete. Data shape: {self.dataset.x_unlabeled.shape}")
    ...
    def _add_point_to_labeled_pool(self, datapoint_id):
        ...
        # maintain index tracking for checkpoint saving
        self.labeled_indices.append(self.unlabeled_indices.pop(datapoint_id))
    ...
    def reset_al_pool(self):
        ...
        self.labeled_indices = list(self.dataset.labeled_indices)
        self.unlabeled_indices = list(self.dataset.unlabeled_indices)
