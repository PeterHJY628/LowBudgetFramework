...
class Cifar100(BaseDataset):
    def __init__(self, cache_folder: str, config: dict, pool_rng, encoded: bool, data_file="cifar100_al.pt"):
    ...
    def _download_data(self, target_to_one_hot=True, test_data_fraction=0.5):
        train = torchvision.datasets.CIFAR100(root=self.cache_folder, train=True, download=True)
        test = torchvision.datasets.CIFAR100(root=self.cache_folder, train=False, download=True)
    ...
    def load_pretext_data(self) -> tuple[Dataset, Dataset]:
        train_dataset = torchvision.datasets.CIFAR100(root=self.cache_folder, train=True, download=True)
        val_dataset = torchvision.datasets.CIFAR100(root=self.cache_folder, train=False, download=True)
        train_dataset.targets = torch.Tensor(train_dataset.targets).long()
        val_dataset.targets = torch.Tensor(val_dataset.targets).long()
